...
<x-app-layout>
    ...
    <div class="py-12 bg-white overflow-auto shadow-xl sm:rounded-lg p-8 sr-table">
      <span class="font-extrabold">SERVICE REQUESTS</span>
      <span class="font-extrabold"> {{ $srs->links() }}</span>
      @error('srNumber')
          <span class="text-red-500">{{ $message }}</span>
        @enderror
        <table class="table text-center w-full" id="datatable">
          <thead class="text-black">
            <tr class="bg-blue-200">
              <th class="p-4"><input type="checkbox" class="form-checkbox h-5 w-5 text-red-600"></th>
              <th class="p-4">SR#</th>
              <th class="p-4">Added By</th>
              <th class="p-4">Open Date</th>
              <th class="p-4">Assigned on</th>
              <th class="p-4">Assigned to</th>
              <th class="p-4">Call Type</th>
              <th class="p-4">Escalation</th>
              <th class="p-4">Type</th>
              <th class="p-4">Severity</th>
              <th class="p-4">Customer Mood</th>
              <th class="p-4">Key Account</th>
              <th class="p-4">Account Type</th>
              <th class="p-4">Model No.</th>
              <th class="p-4">Serial No.1</th>
              <th class="p-4">Serial No.2</th>
              <th class="p-4">Purchased On</th>
              <th class="p-4">Dealer Name</th>
              <th class="p-4">Dealer Mobile No</th>
              <th class="p-4">Warranty</th>
              <th class="p-4">Total Amount</th>
              <th class="p-4">TCR</th>
              <th class="p-4">Status</th>
              <th class="p-4">Cancel SR</th>
              <th class="p-4">Customer Name</th>
              <th class="p-4">Mobile No</th>
              <th class="p-4">WhatsApp No</th>
              <th class="p-4">Email ID</th>
              <th class="p-4">Address</th>
              <th class="p-4">Area</th>
              <th class="p-4">City</th>
              <th class="p-4">District</th>
              <th class="p-4">Landmark</th>
          ...
          @endif
            ...
            <tr>
              <td class="p-4"><input type="checkbox" class="form-checkbox h-5 w-5 text-red-600"></td>
              <td class="p-4">{{ $sr->srNumber }}</td>
              <td>{{ $sr-> user->name }}</td>
              <td>{{ $sr-> created_at }}</td>
              <td>{{ $sr-> srAssigned }}</td>
              <td>{{ $sr-> assignedTo }}</td>
              ...
              <td>{{ $sr-> serialNo1 }}</td>
              <td>{{ $sr-> serialNo2 }}</td>
              <td>{{ $sr-> purchasedOn }}</td>
              <td>{{ $sr-> dealerName }}</td>
              <td>{{ $sr-> dealerMobile }}</td>
              <td>{{ $sr-> warranty }}</td>
        ...
        <table class="table text-center w-full">
          <tbody>
            <tr><td>&nbsp;</td></tr>
              <tr>
                <td>&nbsp;</td>
                <td><input type="text" class="mt-1 block w-full px-4 py-1 rounded filter-input" placeholder="Search by SR#" data-column="1"/></td>
                <td><input type="text" class="mt-1 block w-full px-4 py-1 rounded filter-input" placeholder="Search by Customer Mobile No" data-column="25"/></td>
                <td><input type="text" class="mt-1 block w-full px-4 py-1 rounded filter-input" placeholder="Search by SR Status" data-column="22"/></td>
              </tr>              
            </tbody>
        </table>
        <hr style="padding-top: 40px; margin-top:40px;">
    </div>
    ...
    <div class="py-12">
        ...
        <div class="max-w-full mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-8">
                ...
                <form action="{{ route('store.servicerequest') }}" method="POST">
                  @csrf
                <div class="grid grid-cols-8 gap-4 form-group">
                  ...
                  <div>
                    <label class="block">
                      ...
                      <input disabled type="text" name="srOpenDate" class="mt-1 block w-full px-4 py-1 rounded" style="padding-top: 0.5rem; padding-bottom:0.5rem;" placeholder="{{ $now }}">
                    </label>
                  </div>
                  <div>
                    <label class="block">
                ...
                <div>
                  <label class="block">
                    ...
                    <input type="file" class="mt-1 block w-full rounded">
                  </label>
                </div>
    ...
    <script>
      $(document).ready( function () {
          $('#datatable').DataTable({
            processing: true,
            serverSide : true,
            'ajax' : "{{ route('api.servicerequest.index') }}",
            'cloumns' : [
              {'data' : 'srNumber'},
              {'data' : 'mobileNo'},
              {'data': 'status'}
